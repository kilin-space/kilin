...
2. Write the complete intended `WORKFLOW.md` and `WORKFLOW.yaml` to two private temporary candidate files. Verify by parsing the YAML candidate that every user-supplied prompt and approval question decodes to the exact received string. When a `json` output references a schema by path, write the schema JSON file at the declared relative path beneath the definition candidate's directory (for example `schemas/findings.json` beside the definition candidate) so the publisher can stage it with the package. Do not create or reserve the target package yourself.
3. Publish both candidates only through `scripts/publish-workflow.mjs` beneath the resolved physical skill directory. Pass `--scope project|user`, the selected physical `--scope-root`, the exact current project as `--cwd`, the resolved built CLI, both candidate files, and the exact `.agents/workflows/<id>` target. Invoke arguments directly without a shell. The script validates a staged complete package, rejects symlinked or out-of-scope workflow paths observed during its checks, and atomically publishes the staged package directory without overwriting a non-empty target. It stages every schema file the definition references by path, read relative to the definition candidate's directory, under the same package-relative path. The physical paths must remain stable while the helper runs; stop if another process may be replacing `.agents` or its parent directories. If the target exists, stop; never replace, merge, truncate, rename, or delete it.
4. Delete both private candidate files and any temporary schema files after publication or failure. The publisher removes its staged package on every path.
